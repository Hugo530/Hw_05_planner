...
const createHourEl = (time) =>
  $(`<div class="col-1 hour" id=${time}-hour>${time}</div>`);
...
const createSaveButton = (time) =>
  $(
    `<button class="saveBtn material-icons h-50" type="button" data-bs-toggle="modal" data-bs-target="#Modal" id=${time}-save-btn>save</button>`
  );

const createDelButton = (time) =>
  $(
    `<button class="deleteBtn material-icons h-50" type="button" data-bs-toggle="modal" data-bs-target="#Modal" id=${time}-del-btn>delete</button>`
  );
...
const deleteEvent = (eventForDeletion) => {
  if (
    ...
    !checkForDuplicateIds(eventForDeletion.id)
  ) {
